...
{phinterval} set operations work as expected with arbitrary time spans, enabling operations that are not supported by {lubridate}. For example, the 
intersection of two non-overlapping intervals is an empty time span, called a `<hole>`.
...
implementation of `phint_squash()` was inspired by the [{intervals}](https://CRAN.R-project.org/package=intervals) package by
