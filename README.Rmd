...
To create phintervals with multiple disjoint spans per element, use the `by` 
argument to group intervals. Overlapping or adjacent spans within each group 
...
```{r, include = FALSE}
opts <- options(phinterval.print_max_width = 150)
```
...
The phinterval package is most useful when working with tabular data, such as a 
...
`<phinterval>` column formatting adapts to the available console width. The
`"[2]"` in Greg's employment interval `"{2018-01-01-[2]-2020-04-01}"` indicates
that his employment history is made up of two disjoint spans, with the first
span beginning on 2018-01-01 and the second ending on 2020-04-01. When more
...
Operations on `<phinterval>` vectors behave like those on standard intervals. Here,
we can see that there was a 7-day gap in Greg's employment history:
...
The `<phinterval>` class is a generalization of the `<Interval>` class, meaning
any `<Interval>` can be converted into an equivalent `<phinterval>` and all phinterval 
functions accept either `<Interval>` or `<phinterval>` inputs. The table below 
shows the lubridate functions that have drop-in phinterval replacements.
...
Performing a set-difference may "punch a hole" in a time span, creating a discontinuous
interval.
...
`<phinterval>` data structure (a list of matrices) and the C++ implementation 
of `phint_squash()` were inspired by the [{intervals}](https://CRAN.R-project.org/package=intervals) 
package by Richard Bourgon and Edzer Pebesma. The figures used in this README
were inspired by Davis Vaughan's [{ivs}](https://davisvaughan.github.io/ivs/) 
